...
# доллар $ - конец строки
# [] - любой один вводимый символ из перечисленных в скобках, можно вводить диапазоны 
# [^] - любой один символ кроме перечисленных в скобках 
# \ - экранирование - отменяет спецзначение следующего символа 
# . - любой один символ 
# * - предыдущий шаблон встречается ноль и более раз
# ? - предыдущий шаблон необязателен (0 или 1 раз)
# \+ - предыдущий шаблон обязателен (1 и более раз)
# \{n\} - предыдущий шаблон встречается ровно n раз (нет в awk)
# \{n,m} - предыдущий шаблон встречается ровно n-m раз (нет в awk)
...
file_size=${file_size%?}
# ${} тут - это "вытаскивание" значения из переменной и присваивание его. ${} - расширение переменной, $()присваивание выполнения команды
# ${file_size%?} - если переменная существует присваивает, если её нет - дает ошибку
fi
